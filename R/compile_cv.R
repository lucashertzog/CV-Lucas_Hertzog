...
# compile_cv <- function() {
#   original_cv_lines <- readLines("cv.tex")
#   cv_lines <- c()  # Initialise an empty vector to store the processed lines
#   
#   include_section <- function(tag, include) {
#     section_start <- grep(paste0("%#", tag), original_cv_lines)
#     section_end <- grep(paste0("%%", tag), original_cv_lines)
#     
#     if (length(section_start) == 0 || length(section_end) == 0) {
#       warning(paste("Section markers for", tag, "not found. Skipping this section."))
#       return(character(0))
#     }
#     
#     if (include) {
#       message("Including section: ", tag)  # Debugging message
#       return(original_cv_lines[section_start:section_end])  # Return section lines
#     } else {
#       message("Excluding section: ", tag)  # Debugging message
#       return(character(0))  # Return empty if section is not included
#     }
#   }
#   
#   # Concatenate each section to cv_lines based on flags
...
#     engine = "xelatex")
#   
#   # Rename temp_cv.pdf to cv_hertzog.pdf within the temp directory
#   temp_pdf_path <- file.path("temp", "temp_cv.pdf")
#   renamed_pdf_path <- file.path("temp", "cv_hertzog.pdf")
#   file.rename(temp_pdf_path, renamed_pdf_path)
#   
#   # Move cv_hertzog.pdf from temp to the working directory
#   final_pdf_path <- file.path("cv", "cv_hertzog.pdf")
#   if (file.copy(renamed_pdf_path, final_pdf_path, overwrite = TRUE)) {
#     file.remove(renamed_pdf_path)
#   }
